...
def open_biomass_tile(tile_id, version, y0=2014, y1=2021):
    ...
    ds['land_mask'] = utils.find_matching_records(
        data=land_mask, lats=ds.lat, lons=ds.lon
    )
...
def coarsen_biomass_one_tile(
    ...
    if not (skip_existing and zarr_is_complete(coarse_mapper, check=checks)):
        ...
        coarse_out = coarsen_emissions(
            ds[variables], factor=coarsening_factor, mask_var=variables[0], method='mean').chunk(
            coarse_chunks)
        ...
        coarse_out[variables].to_zarr(
            coarse_mapper, encoding={var: encoding for var in variables}, mode="w", consolidated=True)
    ...
    if not (skip_existing and zarr_is_complete(mapper, check=checks)):
        ...
        with dask.config.set(**{'array.slicing.split_large_chunks': False}):
            ...
            for tile_id in tiles:
                sub = xr.open_zarr(
                    input_tile_template.format(tile_id=tile_id), consolidated=True
                )
