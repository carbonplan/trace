...
def plot_shot(record):
    ...
    plt.plot([-0.05, 0.5], np.array([record.sig_begin_distance, record.sig_begin_distance]), 'r--', label='Signal Beginning')   
    plt.plot([-0.05, 0.5], np.array([record.sig_end_distance, record.sig_end_distance]), 'g--', label='Signal End')

    # plot noise mean and std from GLAH01
    plt.plot([record.noise_mean, record.noise_mean], [bins.min(), bins.max()], '0.5', label='Noise Mean')
    n_sig = 3.5
    ...
    plt.plot([-0.05, 0.5], [record['10th_distance'], record['10th_distance']], 'b--', label='10th Percentile')
    plt.plot([-0.05, 0.5], [record.meanH_distance, record.meanH_distance], 'c--', label='Mean H')
    plt.plot([-0.05, 0.5], [record['90th_distance'], record['90th_distance']], 'm--', label='90th Percentile')
    plt.plot([-0.05, 0.5], [record.ground_peak_distance, record.ground_peak_distance], 'y--', label='Ground Peak')
