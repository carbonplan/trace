...
def main(products=['GLAH01', 'GLAH14']):
    ...
    for p, presults in results.items():
        ...
        print(f'writing {p}')
        print(ds)
        mapper = fsspec.get_mapper(f'gs://carbonplan-scratch/{p.lower()}.zarr')
        mapper.clear()

        _ = ds.to_zarr(mapper, compute=False, mode='w')
        stepsize = chunksize * 100
        recs = ds.dims['record_index']
        for left, right in tqdm(
            zip(range(0, recs, stepsize), range(stepsize, recs + stepsize, stepsize))
        ):
            s = slice(left, right)
